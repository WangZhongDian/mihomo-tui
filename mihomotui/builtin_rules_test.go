...
func TestBuiltInRulesOrderingAndDisable(t *testing.T) {
	...
	if strings.Contains(joined, "builtin-provider-reject") { t.Fatal("disabled provider was emitted") }
	...
	if !(pre >= 0 && user > pre && post > user && match > post) { t.Fatalf("unexpected order: %s", joined) }
}

func TestBuiltInRulesValidationProtectsMatch(t *testing.T) {
	c := defaultConfig()
	c.BuiltInRules[len(c.BuiltInRules)-1].Enabled = false
	if err := c.Validate(); err == nil { t.Fatal("disabled MATCH should fail validation") }
	c = defaultConfig()
	c.PreCustomRules = []string{"MATCH,DIRECT"}
	if err := c.Validate(); err == nil { t.Fatal("custom MATCH should fail validation") }
}
...
func TestLegacyCustomRulesMigration(t *testing.T) {
	...
	if len(c.PreCustomRules) != 1 || c.PreCustomRules[0] != c.CustomRules[0] || len(c.BuiltInRules) == 0 { t.Fatalf("legacy rules not migrated: %#v", c) }
}
